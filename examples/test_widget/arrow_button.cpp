...
*/
QwtArrowButton::QwtArrowButton( int num, Qt::ArrowType arrowType, QWidget *parent, CDATATYPE *pModel ):
    QPushButton( parent )
{
    d_data = new PrivateData;
    d_data->num = qBound( 1, num, MaxNum );
    d_data->arrowType = arrowType;
    d_data->pModel = pModel;

    if (d_data->pModel != NULL ){
        d_data->pModel->push_parent((WM_HMEM)this);
    }
...
void QwtArrowButton::paintEvent( QPaintEvent *event )
{
    ...
    if (d_data->pModel != NULL ){
        char buff[32];
        int val;
        val = d_data->pModel->gain_val(buff);
        qDebug("[%s-%i]%i,%s", __FILE__, __LINE__, val, buff);
...
void QwtArrowButton::keyPressEvent( QKeyEvent *event )
{
    ...
    if (d_data->pModel != NULL ){
        ...
        //if (event->type() == QEvent::KeyRelease){
            if ( (d_data->arrowType == Qt::RightArrow || d_data->arrowType == Qt::UpArrow ) ){
                d_data->pModel->set_IncVal();
            } else {
                d_data->pModel->set_DecVal();
            }
        //}
